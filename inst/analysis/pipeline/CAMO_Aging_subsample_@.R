...
# BA11 and BA47 each split at random into halves and run at two seeds; concordance
# across seeds on the same half measures MCMC reliability, across halves biological
# reproducibility.
#── Clear environment ───────────────────────────────────────────────────────────
rm(list=ls())
...
# (obs - null mean) / (1 - null mean), null from permuting p_B
...
run_single_repetition <- function(rep_num, ba11_data, ba47_data, 
                                  ...
                                  n_perm = 1000, n_cores_per_rep = 8) {

  cat("\n", rep("=", 70), "\n", sep="")
  cat(sprintf("REPETITION %d (using %d cores)\n", rep_num, n_cores_per_rep))
  cat(rep("=", 70), "\n\n", sep="")
  ...
  # Eight MCMC jobs: region x half x seed (1 or 10001)
  mcmc_jobs <- list(
    # BA11: subset1, seed=1
...
N_REPS <- 10
N_CORES_TOTAL <- 40
N_REPS_PARALLEL <- 10  # repetitions run in parallel
...
# Per-repetition mean of one metric over the MCMC or biological comparisons
