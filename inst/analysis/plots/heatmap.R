...
#     concordance[status_vec == "Gain"] <- paste0("Gain in ", group_names[2])
#     concordance[status_vec == "Loss"] <- paste0("Loss in ", group_names[2])
#     
#   } else {
...
#   priority[!is.na(phase_status) & phase_status == "Shifted"] <- 5
#   priority[!is.na(phase_status) & phase_status == "Conserved"] <- 4
#   priority[!is.na(concordance) & concordance == "Concordant" & is.na(phase_status)] <- 3
#   priority[!is.na(concordance) & grepl("^Loss", concordance)] <- 2
#   priority[!is.na(concordance) & grepl("^Gain", concordance)] <- 2
#   priority[is.na(concordance)] <- 1
#   
#   order_idx <- order(priority, max_rho, decreasing = TRUE)
...
#   
#   # Force all categories to appear in legend
#   concordance_ord <- factor(
#     concordance_ord,
#     levels = c("Conserved", loss_label, gain_label)
#   )
#   
...
#       # Calculate circular difference: Group2 - Group1
#       delta <- peak2_ord[i] - peak1_ord[i]
#       
...
#   delta_colors[!is.na(phase_status_ord) & phase_status_ord == "Shifted"] <- "#E63946"
#   delta_colors[!is.na(phase_status_ord) & phase_status_ord == "Conserved"] <- "#06A77D"
#   
#   # Use deltaPhi for all genes (already calculated above)
...
#     filename <- paste0(save_path, "/", gsub("[^A-Za-z0-9]", "_", pathway_name), "_integrated.pdf")
