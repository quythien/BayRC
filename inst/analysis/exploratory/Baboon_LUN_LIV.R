# Baboon lung versus baboon liver: concordance, peak timing, pathway
# enrichment and expected transition counts.
...
results_global1 <- multi_conservation(
  mcmc.merge.list = list(baboon_LIV, baboon_LUN),
  dataset.names = c("baboon_LIV", "baboon_LUN"),
  select.pathway.list = "global",  
  n_perm = 500,
  n_boot = 100,
  output.dir = file.path(output.dir, "LIV_LUN_concordance"),
  use_cpp = TRUE,
  compute_pvalue = TRUE, 
  compute_ci = TRUE 
)
...
rhy   <- detect_rhy(baboon_LIV, baboon_LUN, 0.25)
...
# Rhythmic genes per tissue at BFDR 0.25
rhy   <- detect_rhy(baboon_LIV, baboon_LUN, 0.25)
rhy_summary <- data.frame(
  Category = c("Rhythmic in LIV",
               "Rhythmic in LUN"),
...
# Gain, loss and maintained at BFDR 0.25, then phase within the maintained set
...
# 677 maintained: 335 phase-conserved, 39 phase-shifted, 303 undetermined

#───────────────────────────────────────────────────────────────
# Peak concordance plot for maintained genes, baboon LUN vs LIV
#───────────────────────────────────────────────────────────────
library(ggplot2)
library(dplyr)
library(ggrepel)
...
#───────────────────────────────────────────────────────────────
# ±2 h concordance among maintained genes
#───────────────────────────────────────────────────────────────
...
plot_df <- maintained_df %>%
  rowwise() %>%
  mutate(
    remapped = remap_to_diagonal(Peak_LUN_ZT, Peak_LIV_ZT),
...
p <- ggplot(plot_df, aes(
  ...
  # Identity and ±2 h bands; the remapping makes ±22 h bands unnecessary
  ...
  labs(
    title = "Circadian Peak Concordance: Baboon LUN versus LIVpocampus",
...
ggsave(
  filename = file.path(save_dir, "Baboon_LUN_LIV_Peak_Concordance_0.25_2h_new.pdf"),
  plot = p,
  width = 9,
  height = 8
)
...
################################################################################
# Pathway analysis: Baboon LUN vs LIV
################################################################################

dataset_names <- c("LUN", "LIV")
qvalue_cut <- 0.25
nperm <- 10000
pathway_size_min <- 10
...
result_union <- pathSelect(
  mcmc.merge.list = list(LUN = baboon_LUN, LIV = baboon_LIV),
  pathway.list = kegg.pathway.list_hsa,
  dataset.names = c("LUN", "LIV"),
...
# KEGG Phagosome is added to the significant set for plotting
significant_pathways <- significance_table %>%
  filter(gain_sig | loss_sig | cons_sig) %>%
  pull(pathway) %>%
  union("KEGG Phagosome")
...
result_multiconservation <- multi_conservation(
  ...
  n_perm = 1000,
  n_boot = 1000,
  output.dir = file.path(output.dir, "multiconservation_LUN_LIV"),
  use_cpp = TRUE
)
...
result_multiconservation_filtered <- result_multiconservation %>%
  select(
    Pathway,
    LUN_vs_LIV_AdjustedConcordance,
    LUN_vs_LIV_PValue,
    LUN_vs_LIV_QValue,
    LUN_vs_LIV_GainLossRatio,
  )

relevant_pathways <- result_multiconservation_filtered$Pathway

#---------------------------------------------------------------------------------
# Heatmaps with LUN as dataset A and LIV as dataset B
...
base_path <- file.path(output.dir, "heatmap_baboon_LUN_LIV")
...
cat("\n=== PLOTTING PATHWAYS FOR BABOON LUN vs LIV ===\n")
...
################################################################################
# Expected counts, baboon LUN vs LIV, summed from posterior probabilities
################################################################################

library(dplyr)
library(kableExtra)

pA <- rowMeans(baboon_LUN$rho)   # LUN
...
if ("p_gain" %in% names(trans_outer)) {
  ...
  expected_rhythmic_LUN <- sum(pA, na.rm = TRUE)
  ...
  expected_rhythmic_LUN <- sum(p_rhythmic_LUN, na.rm = TRUE)
  ...
  cat("Expected rhythmic LUN:", round(expected_rhythmic_LUN, 1), "\n")
  cat("Expected rhythmic LIV:", round(expected_rhythmic_LIV, 1), "\n")
  cat("Expected Gain (LIV > LUN):", round(expected_gain, 1), "\n")
  cat("Expected Loss (LUN > LIV):", round(expected_loss, 1), "\n")
  cat("Expected Conserved:", round(expected_cons, 1), "\n")
}

# Threshold or NA when trans_outer does not carry it
safe_get <- function(x) {
  if (is.null(x)) return(NA)
  unname(x)
}

thr_LUN  <- safe_get(trans_outer$tau_rhythmic_A)
...
genome_wide_expected <- data.frame(
  Category = c(
    "Total Genes",
    "Expected Rhythmic in Baboon LUN",
...
################################################################################
# Peak concordance figure, as drawn by run_concordance_LUN_LIV.R; needs the
# objects above in the session.
################################################################################
