# GTEx brain, younger vs older donors: concordance and phase inference, then the baboon/human LIV pathway workflow.
rm(list = ls())
...
younger <- list(
  rho = younger_symbol$rho,
  phi = younger_symbol$phi
)

older <- list(
  rho = older_symbol$rho,
  phi = older_symbol$phi
)
...
results_global1 <- multi_conservation(
  mcmc.merge.list   = list(younger, older),
  dataset.names     = c("Younger_PRC", "Older_PRC"),
  select.pathway.list = "global",
  n_perm = 500,
  n_boot = 100,
  output.dir = file.path(output.dir, "concordance_YO"),
  use_cpp = TRUE,
  compute_pvalue = FALSE, 
  compute_ci = FALSE 
)

rhy <- detect_rhy(younger, older, 0.25)

#---------------------------------------------------------------------------------
# Outer (rhythmic) and inner (phase) analysis
#---------------------------------------------------------------------------------
pA <- rowMeans(younger$rho)
pB <- rowMeans(older$rho)

trans_outer <- transition_classify(pA, pB, bfdr_alpha = 0.25)
phase_inner <- phase_infer(
  phi_matrix1      = younger$phi,
  phi_matrix2      = older$phi,
...
#---------------------------------------------------------------------------------
# Maintained plot: Peak Concordance Plot - Baboon LIV vs Human LIV
#---------------------------------------------------------------------------------
...
maintained_df <- data.frame(
  Gene           = gene_names,
  Peak_BaboonLIV = phase_inner$peak1,
  Peak_HumanLIV  = phase_inner$peak2,
...
maintained_df <- maintained_df %>%
  ...
  mutate(
    Peak_BaboonLIV_ZT = to_zt(Peak_BaboonLIV),
    Peak_HumanLIV_ZT  = to_zt(Peak_HumanLIV)
  )
...
cat("\n[Global Summary - Baboon LIV vs Human LIV]\n")
...
#───────────────────────────────────────────────────────────────
# Share of maintained genes with peaks within ±3 h
#───────────────────────────────────────────────────────────────
...
maintained_df <- maintained_df %>%
  mutate(
    peak_diff          = calculate_peak_difference(Peak_BaboonLIV_ZT, Peak_HumanLIV_ZT),
...
plot_df <- maintained_df %>%
  rowwise() %>%
  mutate(
    remapped = remap_to_diagonal(Peak_BaboonLIV_ZT, Peak_HumanLIV_ZT),
    Peak_BaboonLIV_ZT_plot = remapped$x,
    Peak_HumanLIV_ZT_plot  = remapped$y
  ) %>%
  select(-remapped)
...
p <- ggplot(plot_df, aes(
  x = Peak_BaboonLIV_ZT_plot,
  y = Peak_HumanLIV_ZT_plot,
  color = phase_class
)) +
  ...
  labs(
    title    = "Circadian Peak Concordance: Baboon LIV versus Human LIV",
    subtitle = subtitle_text,
    x        = "Peak Hour – Baboon LIV (ZT)",
    y        = "Peak Hour – Human LIV (ZT)",
    color    = "Phase class"
  ) +
  scale_x_continuous(
    breaks = seq(-6, 18, 6),
...
# Save
#───────────────────────────────────────────────────────────────
save_dir <- file.path(output.dir, "figure/baboon_human_LIV")
dir.create(save_dir, recursive = TRUE, showWarnings = FALSE)

ggsave(
  file.path(save_dir, "Baboon_Human_LIV_Peak_Concordance_0.15.pdf"),
  plot = p, width = 9, height = 8
)

################################################################################
# COMPLETE WORKFLOW: Baboon LIV vs Human LIV Pathway Analysis
################################################################################

# Set parameters
dataset_names <- c("Baboon_LIV", "Human_LIV")
qvalue_cut <- 0.25
nperm <- 10000
pathway_size_min <- 10
...
result_union <- pathSelect(
  mcmc.merge.list = list(Baboon_LIV = baboon_LIV, Human_LIV = human_LIV),
  pathway.list = kegg.pathway.list_hsa,
  dataset.names = c("Baboon_LIV", "Human_LIV"),
...
cat("  Gain-enriched (Human_LIV):  ", sum(result_gain$results$Significant), "\n")
cat("  Loss-enriched (Human_LIV):  ", sum(result_loss$results$Significant), "\n")
...
print_pathway_summary <- function(result_obj, 
  ...
  # Expected union count and its share of pathway size
...
result_multiconservation <- multi_conservation(
  ...
  n_perm = 1000,
  n_boot = 1000,
  output.dir = file.path(output.dir, "multiconservation_filtered_LIV"),
  use_cpp = TRUE
)
...
result_multiconservation_filtered <- result_multiconservation %>%
  select(
    Pathway,
    Baboon_LIV_vs_Human_LIV_AdjustedCongruence,
    Baboon_LIV_vs_Human_LIV_PValue,
    Baboon_LIV_vs_Human_LIV_QValue,
    Baboon_LIV_vs_Human_LIV_GainLossRatio
  )
...
base_path <- file.path(output.dir, "heatmap_baboon_human_LIV")
dir.create(base_path, showWarnings = FALSE, recursive = TRUE)

###############################################################
# PLOT EACH PATHWAY
###############################################################
pA <- rowMeans(human_LIV$rho)
...
for (pathway in pathways_to_plot) {
  ...
  overlap <- intersect(rownames(baboon_LIV$rho), pathway_genes)
  ...
  tryCatch({
    ...
    plot_heatmap(
      ...
      group_names = c("Human LIV", "Baboon LIV"),
      versions = "both",
      save_path = out_dir
    )
...
################################################################################
# GENOME-WIDE EXPECTED COUNTS: summed posterior probabilities
################################################################################
...
genome_wide_expected <- data.frame(
  Category = c(
    ...
    "Expected Rhythmic in Human LIV",
...
cat("Baboon LIV vs Human LIV\n")
cat("========================================\n\n")

print(genome_wide_expected)

# Gain + loss + conserved is the expected union
cat("\n=== VERIFICATION ===\n")
cat("Expected Gain + Loss + Conserved =", 
    round(expected_gain + expected_loss + expected_cons, 1), "\n")

# Circular phase plot of maintained clock genes, mean ±1 circular SD
library(ggplot2)
library(dplyr)
library(tidyr)
library(circular)
...
results <- data.frame(
  ...
  Mean_Baboon_hr = sapply(conserved_genes, function(g) circular_mean_24h(baboon_LIV$phi[g,])),
  Mean_Human_hr = sapply(conserved_genes, function(g) circular_mean_24h(human_LIV$phi[g,])),
  SD_Baboon_hr = sapply(conserved_genes, function(g) {
    r <- circular((baboon_LIV$phi[g,] / 24) * 2 * pi, units = "radians")
    (sd.circular(r) / (2*pi)) * 24
  }),
  SD_Human_hr = sapply(conserved_genes, function(g) {
    r <- circular((human_LIV$phi[g,] / 24) * 2 * pi, units = "radians")
    (sd.circular(r) / (2*pi)) * 24
  })
)
...
save_dir <- file.path(output.dir, "figure/baboon_human_LIV")
dir.create(save_dir, recursive = TRUE, showWarnings = FALSE)
ggsave(file.path(save_dir, "Baboon_Human_LIV_circular_phase_plot.pdf"), p, width = 12, height = 10, dpi = 800)
ggsave(file.path(save_dir, "Baboon_Human_LIV_circular_phase_plot.png"), p, width = 12, height = 10, dpi = 800)
...
# Transition status of ribosome genes
