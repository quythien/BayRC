# SAME SPECIE  (Baboon ILE vs Human ILE)
...
#---------------------------------------------------------------------------------
# BRAIN/ILE OBJECTS (HERE: ILE Baboon vs ILE Human)
#---------------------------------------------------------------------------------
...
human_ILE <- list(
  rho = mcmc_data_human$ILE,
  phi = mcmc_phi_human$ILE
)
...
results_global1 <- multi_conservation(
  mcmc.merge.list   = list(baboon_ILE, human_ILE),
  dataset.names     = c("Baboon_ILE", "Human_ILE"),
  select.pathway.list = "global",
  n_perm            = 1000,
  n_boot            = 1000,
  output.dir        = file.path(output.dir, "concordance_ILE"),
  use_cpp           = TRUE
)

rhy <- detect_rhy(baboon_ILE, human_ILE, 0.25)

#---------------------------------------------------------------------------------
# Outer (rhythmic) and inner (phase) analysis
#---------------------------------------------------------------------------------

# dataset1 = Baboon ILE, dataset2 = Human ILE
pA <- rowMeans(baboon_ILE$rho)
pB <- rowMeans(human_ILE$rho)

trans_outer <- transition_classify(pA, pB, bfdr_alpha = 0.25)

phase_inner <- phase_infer(
  phi_matrix1      = baboon_ILE$phi,
  phi_matrix2      = human_ILE$phi,
...
#---------------------------------------------------------------------------------
# Maintained plot: Peak Concordance Plot - Baboon ILE vs Human ILE
#---------------------------------------------------------------------------------
...
maintained_df <- data.frame(
  Gene           = gene_names,
  Peak_BaboonILE = phase_inner$peak1,
  Peak_HumanILE  = phase_inner$peak2,
...
maintained_df <- maintained_df %>%
  ...
  mutate(
    Peak_BaboonILE_ZT = to_zt(Peak_BaboonILE),
    Peak_HumanILE_ZT  = to_zt(Peak_HumanILE)
  )
...
cat("\n[Global Summary - Baboon ILE vs Human ILE]\n")
...
maintained_df <- maintained_df %>%
  mutate(
    peak_diff          = calculate_peak_difference(Peak_BaboonILE_ZT, Peak_HumanILE_ZT),
...
plot_df <- maintained_df %>%
  rowwise() %>%
  mutate(
    remapped = remap_to_diagonal(Peak_BaboonILE_ZT, Peak_HumanILE_ZT),
    Peak_BaboonILE_ZT_plot = remapped$x,
    Peak_HumanILE_ZT_plot  = remapped$y
  ) %>%
  select(-remapped)
...
p <- ggplot(plot_df, aes(
  x = Peak_BaboonILE_ZT_plot,
  y = Peak_HumanILE_ZT_plot,
  color = phase_class
)) +
  ...
  labs(
    title    = "Circadian Peak Concordance: Baboon ILE versus Human ILE",
    subtitle = subtitle_text,
    x        = "Peak Hour – Baboon ILE (ZT)",
    y        = "Peak Hour – Human ILE (ZT)",
    color    = "Phase class"
  ) +
  scale_x_continuous(
    breaks = seq(-6, 18, 6),
...
# Save
#───────────────────────────────────────────────────────────────
save_dir <- file.path(output.dir, "figure/baboon_human_ILE")
dir.create(save_dir, recursive = TRUE, showWarnings = FALSE)

ggsave(
  file.path(save_dir, "Baboon_Human_ILE_Peak_Concordance_0.15.pdf"),
...
#───────────────────────────────────────────────────────────────
################################################################################
# COMPLETE WORKFLOW: Baboon ILE vs Human ILE Pathway Analysis
################################################################################

# Set parameters
dataset_names <- c("Baboon_ILE", "Human_ILE")
qvalue_cut <- 0.25
nperm <- 10000
pathway_size_min <- 10
...
result_union <- pathSelect(
  mcmc.merge.list = list(Baboon_ILE = baboon_ILE, Human_ILE = human_ILE),
...
cat("  Gain-enriched (Human_ILE):  ", sum(result_gain$results$Significant), "\n")
cat("  Loss-enriched (Human_ILE):  ", sum(result_loss$results$Significant), "\n")
...
result_multiconservation <- multi_conservation(
  ...
  n_perm = 1000,
  n_boot = 1000,
  output.dir = file.path(output.dir, "multiconservation_filtered_ILE"),
  use_cpp = TRUE
)
...
result_multiconservation_filtered <- result_multiconservation %>%
  select(
    Pathway,
    Baboon_ILE_vs_Human_ILE_AdjustedCongruence,
    Baboon_ILE_vs_Human_ILE_PValue,
    Baboon_ILE_vs_Human_ILE_QValue,
    Baboon_ILE_vs_Human_ILE_GainLossRatio
  )
...
###############################################
# PATHWAY LIST TO PLOT - BABOON vs HUMAN ILE
###############################################
...
base_path <- file.path(output.dir, "heatmap_baboon_human_ILE")
dir.create(base_path, showWarnings = FALSE, recursive = TRUE)

###############################################################
# PLOT EACH PATHWAY
###############################################################
pA <- rowMeans(human_ILE$rho)
...
cat("\n=== PLOTTING PATHWAYS FOR BABOON vs HUMAN ILE ===\n")
...
for (pathway in pathways_to_plot) {
  ...
  tryCatch({
    ...
    plot_heatmap(
      ...
      group_names = c("Human ILE", "Baboon ILE"),
      versions = "both",
      save_path = out_dir
    )
...
# For Baboon ILE vs Human ILE analysis
pA <- rowMeans(baboon_ILE$rho)
pB <- rowMeans(human_ILE$rho)
...
genome_wide_expected <- data.frame(
  Category = c(
    ...
    "Expected Rhythmic in Human ILE",
...
results <- data.frame(
  ...
  Mean_Baboon_hr = sapply(conserved_genes, function(g) circular_mean_24h(baboon_ILE$phi[g,])),
  Mean_Human_hr = sapply(conserved_genes, function(g) circular_mean_24h(human_ILE$phi[g,])),
  SD_Baboon_hr = sapply(conserved_genes, function(g) {
    r <- circular((baboon_ILE$phi[g,] / 24) * 2 * pi, units = "radians")
    (sd.circular(r) / (2*pi)) * 24
  }),
  SD_Human_hr = sapply(conserved_genes, function(g) {
    r <- circular((human_ILE$phi[g,] / 24) * 2 * pi, units = "radians")
    (sd.circular(r) / (2*pi)) * 24
  })
)
...
save_dir <- file.path(output.dir, "figure/baboon_human_ILE")
dir.create(save_dir, recursive = TRUE, showWarnings = FALSE)
ggsave(file.path(save_dir, "Baboon_Human_ILE_circular_phase_plot.pdf"), p, width = 12, height = 10, dpi = 800)
ggsave(file.path(save_dir, "Baboon_Human_ILE_circular_phase_plot.png"), p, width = 12, height = 10, dpi = 800)
