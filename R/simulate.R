# Default scenario grid for CBt_sim_data; dA/dPhase/dM are offsets from group 1.
...
CBt_sim_data = function(P = 24,
                        ...
                        G = NULL,
                        n = 30,
                        fixed_tp = TRUE, # |t_p| = sigma.t, half the signs negative
                        sigma.t = 2,
                        n.sigma.t = 16,
                        p.sigma.t = NULL, # proportion of samples with time error
                        # one of n.sigma.t or p.sigma.t must be NULL
                        Params = list(A = 0.5, phi = 0, M = 5, sigma = 1),
                        TimePoints = NULL,
                        PhaseClust = NULL){
  # dParam columns must be named dA, dPhase, dM
  ...
  TOJR.dParam.long = lapply(seq_along(dParam), function(j){
    # repeat scenario row a G[a] times
  ...
  if(is.null(TimePoints)){
    t = runif(n, 0, P) # observed time
    # choose the samples with t_p != 0
    ...
    if(fixed_tp){
      # half of the shifted samples get a negative t_p
      n.t.p = sum(t.p.ind)
