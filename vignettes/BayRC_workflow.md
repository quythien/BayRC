...
## Multi-tissue pathway profiles

Use `plot_pathway_profiles()` for paired rhythmicity and phase profiles (Figure 6C/D). The [pathway profiles tutorial](pathway_profiles.md) covers posterior summaries, tissue and gene grouping, coverage thresholds, sparse pathways and a runnable example. Use `pathway_phase_hierarchy()` and `plot_pathway_phase_hierarchy()` to inspect the underlying phase trees.
